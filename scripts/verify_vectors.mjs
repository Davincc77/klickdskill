...
// This runner lives in scripts/; the repository root (which holds tests/) is its parent.
const __root = dirname(__dir);
...
for (const { path, label } of suites) {
  const { passed, failed, skipped } = await runSuite(join(__root, path), label);
  totalPassed  += passed;
  totalFailed  += failed;
...
// v4 preview suite (additive, NOT GA)
{
  const { passed, failed, skipped } = await runV40PreviewSuite(
    join(__root, 'tests/vectors_v40_preview.json'),
...

{
  const { passed, failed, skipped } = await runV40GaStrictSuite(
    join(__root, 'tests/vectors_v40_ga.json'),
    join(__root, 'tests/negative_vectors_v40_ga.json'),

...
- [`docs/rfcs/RFC-010-pack-memory-compression.md`](../rfcs/RFC-010-pack-memory-compression.md) — RFC-010 `compressed_memory` (v4.2 Draft). Since 2026-05-28 every artefact under `examples/v4.1/x-klickd-skills/{lite,pro}/` carries an RFC-010 draft/preview block at `x_klickd_pack.structured_memory.compressed_memory`. The block is **NOT** a GA runtime guarantee; it is a draft surface so reviewers can compare RFC-010 against the v4.1 candidate catalogue *before* any v4.2 normative wording, schema, or pack field lands. Promotion to `ship_ready` does NOT depend on RFC-010 (the C-001..C-022 gates above remain the merge contract).

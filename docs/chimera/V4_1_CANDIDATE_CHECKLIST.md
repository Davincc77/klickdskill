...
- [ ] **C-003 — Filename.** The proposed canonical filename does not collide with an existing entry under [`examples/v4/starter-skills/manifest.json`](../../examples/v4/starter-skills/manifest.json) unless the candidate is explicitly a sub-profile of that pack (e.g. `work-lite` is a profile of `x.klickd/work`, NOT a new pack).

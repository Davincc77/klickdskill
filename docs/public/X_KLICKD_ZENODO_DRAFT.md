...
- The v4.1 candidate `x.klickd` artefact tree under `examples/v4.1/x-klickd-skills/` — Lite tier (8 packs at the time of this draft) and Pro tier (34 packs at the time of this draft). The in-repo path is already codename-free (renamed 2026-05-28); the deposit ZIP can capture the directory verbatim. See §7.1 for the codename-bearing paths that still require action before sealing the bundle.
...
1. **Rename in repo first (cleanest).** Open a separate PR that renames the codename-bearing in-repo paths to `x.klickd`-named equivalents (e.g. internal `examples/v4.1/<codename>-skills/` → `examples/v4.1/x-klickd-skills/`; the v4.1 planning RFC → an `x-klickd`-named filename). Merge before tagging v4.1 GA. The deposit bundle then naturally contains only clean paths. *Status 2026-05-28: the artefact tree rename to `examples/v4.1/x-klickd-skills/` has been applied. The v4.1 planning RFC filename and the internal `docs/chimera/` planning track have NOT been renamed yet — option 2 or 3 still applies to those paths.*

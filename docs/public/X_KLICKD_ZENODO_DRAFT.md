...
1. **Rename in repo first (cleanest).** Open a separate PR that renames the codename-bearing in-repo paths to `x.klickd`-named equivalents (e.g. internal `examples/v4.1/<codename>-skills/` → `examples/v4.1/x-klickd-skills/`; the v4.1 planning RFC → an `x-klickd`-named filename). Merge before tagging v4.1 GA. The deposit bundle then naturally contains only clean paths. *Status 2026-05-28: the artefact tree rename to `examples/v4.1/x-klickd-skills/` has been applied. The v4.1 planning RFC filename and the internal-codename planning track have NOT been renamed yet — option 2 or 3 still applies to those paths.*
